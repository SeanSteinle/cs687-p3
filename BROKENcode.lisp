...
in function form (X) rather than just X."
  (if (= size 1)
      (list "r" (list (list (random-elt *terminal-set*)))) ; if size = 1, return random terminal
      (let ((q (make-queue))
        ...
        (while (< (+ count (length q)) size) nil  ; ADD NONTERMINALS UNTIL SIZE IS HIT
          (let* ((new-term (copy-list (random-elt *nonterminal-set*)))
            ...
            (incf count)
            (setf (elt (first child-ref) (second child-ref)) (append (elt (first child-ref) (second child-ref)) new-list))
            ;; (dotimes (j (second new-term))
            ;;   (enqueue (list (elt (first child-ref) (second child-ref)) (1+ j)) q)
            ;;   )
            (print r-node)
            (enqueue (list (elt (first child-ref) (second child-ref)) 1) q)
            ;; (dotimes (j (second new-term))
...
(defmacro progn3 (arg1 arg2 arg3)
